...
\begin{etape}
 The \texttt{StudentGroupLoader} class (provided in the \texttt{src} directory) has a method \texttt{loadTsvFile(...)} that takes as argument a text file (one student per line; its identifier, a tabulation, its grade) for creating an instance of \texttt{StudentGroup}. Why is the method \texttt{loadTsvFile(...)} declared as ``\texttt{static}''? 
