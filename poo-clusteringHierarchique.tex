...
\begin{etape}
 Configure your editor so that the \texttt{.java} source files from the \texttt{src} directory are compiled into \texttt{.class} files into the \texttt{bin} directory.
\end{etape}
...
As we will see, there is no need to distinguish the simple and the complex clusters as subclasses of \texttt{ClusterOfStudents}.
...
\begin{etape}
 ...
 This list is obvisouly empty for simple clusters, and non-empty for complex clusters.
 ...
 This list is never empty; it contains a single element for simple clusters, and two elements or more for complex clusters.
\end{etape}
