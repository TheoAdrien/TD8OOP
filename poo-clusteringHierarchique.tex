...
\subsection{Class \texttt{StudentGroup}: inheritance and static methods}


\begin{etape}
 Create a class \texttt{StudentGroup} that represents a set of students. \texttt{StudentGroup} is a sub-class of \texttt{java.util.ArrayList}\footnoteURL{http://docs.oracle.com/javase/7/docs/api/java/util/ArrayList.html}. Make sure to read the documentation for \texttt{ArrayList}, you will need it soon.
Please note that \texttt{ArrayList} is a generic class, whereas all the elements of a \texttt{StudentGroup} instance are composed of instances of \texttt{Student}, so you will need to state that \texttt{StudentGroup} is a subclass of \texttt{ArrayList<Student>}. 
\end{etape}

\begin{etape}
 In the \texttt{StudentGroup}'s \texttt{main(...)} method, create an instance \texttt{m1bioinfo} of  \texttt{StudentGroup}, and add the members \texttt{riri}, \texttt{fifi}, \texttt{geo}, \texttt{donald} and \texttt{loulou} (respect this order so that the highest and lowest grades are in the middle of the list. The idea here is to avoid having the students almost sorted for the clustering).
\end{etape}

\begin{etape}
 Add the methods \texttt{getMinGrade()}, \texttt{getMaxGrade()} et \texttt{getAverageGrade() to the class \texttt{StudentGroup}}.
...
Feel glad to have declared \texttt{StudentGroup} as a subclass of \texttt{ArrayList}.
\end{etape}

\begin{etape}
 In the \texttt{main(...)} method of \texttt{StudentGroup}, add the code for printing the lowest grade, the highest and the average for \texttt{m1bioinfo}.
\end{etape}

\begin{etape}
 The \texttt{StudentGroupLoader} class (\fixme{fournie sur l'ENT}) has a method \texttt{loadTsvFile(...)} that takes as argument a text file (one student per line; its identifier, a tabulation, its grade) for creating an instance of \texttt{StudentGroup}. Why is the method \texttt{loadTsvFile(...)} declared as ``\texttt{static}''? 
...
\begin{etape}
 ...
 \begin{itemize}
  ...
  \item a constructor for complex clusters before clustering \texttt{ClusterOfStudents(StudentGroup aStudentGroup)}.
 \end{itemize}
\end{etape}
...
\begin{etape}
 Should we have declared \texttt{ClusterOfStudents} as a subclass of \texttt{StudentGroup}?
 Are theses classes' internal structures compatible?
 Are there \texttt{StudentGroup}'s attribute or methods for which such an inheritance would make sense?
\end{etape}
